...
void vendor_load_properties()
{
    ...
    if (device.size() == 0) {
        ERROR("Could not detect device, forcing a7y17lte");
        device = "a7y17lte";
    }

    name = device + "xx";

    description = name + "-user 7.1.2 NJH47F " + bl_model + bl_build + " release-keys";
    fingerprint = "samsung/" + name + "/" + device + ":7.1.2/NJH47F/" + bl_model + bl_build + ":user/release-keys";
